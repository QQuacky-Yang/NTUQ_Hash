...
Original file is located at
    https://colab.research.google.com/drive/1SP8taDpqWswXxRpQF2xiEedkmRed0nQd
"""
...
def _symmetric_normalized_laplacian(A: np.ndarray) -> np.ndarray:
    """Symmetric normalized Laplacian: L^{sn} = D^{-1/2} L D^{-1/2} = I - D^{-1/2} A D^{-1/2}"""
    deg = A.sum(axis=1)
    deg_inv_sqrt = np.where(deg > 0, 1.0 / np.sqrt(deg), 0.0)
    D_inv_sqrt = np.diag(deg_inv_sqrt)
    return np.eye(A.shape[0]) - D_inv_sqrt @ A @ D_inv_sqrt

def _random_walk_normalized_laplacian(A: np.ndarray) -> np.ndarray:
...
def ctqw_hash(bits: str, n: int = 15, t: float = np.pi,
              k: int = 12, scale: int = 20000) -> str:
    """
    Enhanced CTQW hash with four quantum operators:
      U00: Adjacency matrix (A)
      U01: Laplacian matrix (L = D - A)
      U10: Symmetric normalized Laplacian (L^{sn})
      U11: Random walk normalized Laplacian (L^{rw})

    Each pair of message bits selects one operator. Message length must be even.
    If odd, a '0' is appended to make it even.

    Args:
        bits:  bitstring message, e.g. "101001..."
        n:     number of vertices in P_n
        t:     evolution time for all operators (default π)
    ...
    """
    # Pad with zero if message length is odd
    if len(bits) % 2 != 0:
        bits += '0'
    ...
    L_sym = _symmetric_normalized_laplacian(A)
    L_rw = _random_walk_normalized_laplacian(A)

    # Precompute all unitaries
    U_dict = {
        '00': _unitary_from_hermitian(A, t),     # U00: Adjacency matrix
        '01': _unitary_from_hermitian(L, t),     # U01: Laplacian matrix
        '10': _unitary_from_hermitian(L_sym, t), # U10: Symmetric normalized
        '11': _unitary_from_hermitian(L_rw, t)   # U11: Random walk normalized
    }
    ...
    # Apply per message bit pair
    num_pairs = len(bits) // 2
    for i in range(num_pairs):
        bit_pair = bits[2*i:2*i+2]  # Get two consecutive bits
        U = U_dict[bit_pair]        # Select corresponding operator
        state = U @ state           # Apply evolution
...
def collision_test(trials: int = 10000, msg_len: int = 12,
                   n: int = 15, t: float = np.pi,
                   k: int = 12, scale: int = 100000000) -> None:
    """
    Strict 1-bit-flip collision test with enhanced operators:
    ...
    for _ in range(trials):
        ...
        h1 = ctqw_hash(bits, n=n, t=t, k=k, scale=scale)
        pos = random.randrange(len(bits))
        flipped_bits = bits[:pos] + ('1' if bits[pos] == '0' else '0') + bits[pos+1:]
        h2 = ctqw_hash(flipped_bits, n=n, t=t, k=k, scale=scale)
        if h1 == h2:
            collisions += 1
...
def run_omega_test_flip_one_bit(
  iterations: int = 1000,
  n: int = 15, t: float = np.pi, k: int = 12,
  ...
  for t in range(iterations):
    ...
    h1 = ctqw_hash(m1, n=n, t=t, k=k, scale=scale)
    h2 = ctqw_hash(m2, n=n, t=t, k=k, scale=scale)
...
if __name__ == "__main__":
  ...
  # Single hash
  h = ctqw_hash(msg_bits, n=n, t=np.pi, k=k, scale=scale)
  ...
  collision_test(trials=iter, msg_len=len(msg_bits), n=n, t=np.pi, k=k, scale=scale)

  # ω test (paper Sec. 3.5)
  omega = run_omega_test_flip_one_bit(
    iterations=iter, n=n, t=np.pi, k=k,
